...
class MahdiYdb {
    ...
    protected $incompatible_modes = [ 'NO_ZERO_DATE',
                                      'ONLY_FULL_GROUP_BY',
                                      'STRICT_TRANS_TABLES',
                                      'STRICT_ALL_TABLES',
                                      'TRADITIONAL' ];
    ...
    function set_prefix( $prefix, $set_table_names = false, $tbname = [ ] ) {

        if( $set_table_names ) {
            foreach( $this->tables() as $tb )
                if( in_array( $tb, $tbname ) ) {
                    $this->query( $this->dbh, sprintf( 'RENAME TABLE %1$s TO %2$s%1$s', $tb, $prefix ) );
                }
        }
    ...
    function select( $db, $dbh = null ) {
        ...
        if( !$success ) {
            $this->ready = false;
            $this->bail( sprintf( '<h1>Can&#8217;t select database</h1><p>We were able to connect to the database server (which means your username and password is okay) but not able to select the <code>%1$s</code> database.</p>', htmlspecialchars( $db, ENT_QUOTES ), htmlspecialchars( $this->dbuser, ENT_QUOTES ) ), 'db_select_fail' );
    ...
    function real_escape( $string ) {
        ...
        $this->bail( "$class must set a database connection for use with escaping.", "real_escape_fail" );
    ...
    function prepare( $query, $args ) {
        ...
        if( strpos( $query, '%' ) === false ) {
            $this->bail( 'The query argument of db::prepare must have a placeholder.', "db_prepare_fail" );
        }
    ...
    function db_connect( $allow_bail = true ) {
        ...
        if( !$this->dbh && $allow_bail ) {
            ...
            $this->bail( sprintf( "<h1>Error establishing a database connection</h1>", htmlspecialchars( $this->dbhost, ENT_QUOTES ) ), 'db_connect_fail' );
    ...
    function check_connection( $allow_bail = true ) {
        ...
        $this->bail( sprintf( "<h1>Error reconnecting to the database</h1>", htmlspecialchars( $this->dbhost, ENT_QUOTES ) ), 'db_connect_fail' );
