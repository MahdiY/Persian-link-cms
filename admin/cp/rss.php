...
$msg = "";		
	if(isset($_GET['active']) && isset($_GET['id'])){
		$sql = $db->update('_feeds' , array('status'=> intval($_GET['active'])) , array('id'=> intval($_GET['id'])));
		if($sql){
			header("Location: panel.php?act=feeds");
		}
	}
	if(isset($_GET['delet'])){
		$db->delete('_feeds' , array('id'=>intval($_GET['delet'])));	
		header("Location: panel.php?act=feeds");
	}

	if(isset($_POST['save'])){
		if(filter_var($_POST['url'], FILTER_VALIDATE_URL)){
			$addfeed = $db->insert('_feeds' , array('id'=>'' ,'title'=>$_POST['title'] ,'link'=>$_POST['url'] ,'status'=>$_POST['status']));
			if ( $addfeed ){
...
echo '<div class="row">
	...
	$sql = $db->get_results("select * from `_feeds`" , ARRAY_A);
	if($db->num_rows > 0){
	?>
...
if($row['status']==1){
	...
	<tr class="default">
			...
			<td>
			<a class="btn <?php echo $activebtn;?> btn-xs" href="panel.php?act=feeds&active=<?php echo $code;?>&id=<?php echo $row['id'];?>"><i class="fa fa-check"></i> <?php echo $showtext;?></a>
			<a class="btn btn-info btn-xs" title="مشاهده لینک" target="_blank" href="<?php echo $row['link']; ?>" title=""><i class="fa fa-link"></i> مشاهده لینک</a>
			<a class="btn btn-danger btn-xs" title="حذف" href="panel.php?act=feeds&delet=<?php echo $row['id']; ?>"><i class="fa fa-times"></i> حذف</a>
			</td>
	</tr>
